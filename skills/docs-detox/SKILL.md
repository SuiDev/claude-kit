...
12. **横文字メタファーを日本語に戻す**。「思考のOSをアップデート」「人生をハック」「習慣をインストール」「マインドをリファクタリング」を、「考え方を変える」「工夫する」「習慣をつける」「考え方を整理する」に戻す。「コンテキスト」「アライメント」「コミュニケーション」のような普通の日本語に戻せるカタカナ語も、戻す。戻すのは日本語で言える語だけで、技術用語は次の13で扱う。
13. **技術用語を訳しすぎない**。エンジニアが英語で口にする語は英語のまま置く。cold start を「初期起動遅延」、provisioned concurrency を「事前確保済み同時実行数」、payload を「積載物」に訳し替えない。地の文は普通の日本語で書き、技術用語だけ英語で残す。定着した日本語がないときに造語しないこと。識別子、API名、CLIのフラグ、設定キー、ファイルパス、環境変数、エラーメッセージ、ログ出力は原文のまま再現する。ただしリポジトリが日本語表記で統一されているならそちらが優先。禁止例と判断手順は `references/terminology.md`。
14. **形容詞や心情に「」を使わない**。固有名詞か直接引用以外で「」を使わない。「軽い」「動けばヨシ」のような普通の語を引用符で囲まない。
15. **記号のアーティファクトを潰す**。全角ダッシュ ── はコロン・改行・読点に置換。中黒並列 A・B・C は「と」「や」で繋ぐ。`**` 残骸、コロン後の半角スペース、装飾絵文字(🚀🎯✨💡)の均等撒布は検索 → 全置換。

## クイックチェック

書き終わったら、上から順に確認する。何より音読が早い。目で読むと気付かない違和感が、口に出すと一気に出てくる。黙読でも、息が続かない箇所、読点を打ちたい箇所、リズムが揃いすぎている箇所を見る。
...
- [ ] 技術用語を訳しすぎていないか?(cold start → 初期起動遅延、payload → 積載物)
- [ ] 定訳のない語に造語を当てていないか?(限流、許可名簿、無界)
- [ ] 識別子、CLIのフラグ、エラーメッセージ、ログを訳したり言い換えたりしていないか?
- [ ] 英語固有名詞をフル正式名称で律儀に繰り返していないか?
- [ ] 効果をぼかす動詞(効く、刺さる、生きる)で、何がどう変わるかを書かずに済ませていないか?
...
- `references/terminology.md` — 技術用語の訳しすぎ対策。判断手順、造語の禁止、原文のまま置くもの。
